...
const setupHttpServer = (logger: any) => {
  ...
  const server = httpServer
    ...
    .on('error', (e) => {
      logger.error({
        error: e.stack,
        message: `Auth FATAL Error: ${e.name} ${e.message} - Shutting down service...`,
      })
      server.close()
    })
...
//   const secureServer = setupHttpsServer(console)
  return setupHttpServer(console)
}
